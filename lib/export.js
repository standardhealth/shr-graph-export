...
class GraphExporter {
  ...
  collectPrimitiveInfo(node, valueSetConstraints, codeConstraints) {
    let description = [];
    for (const constraint of valueSetConstraints) {
      description.push(`${constraint.valueSet} (${constraint.bindingStrength})`);
    }
    for (const constraint of codeConstraints) {
      description.push(constraint.code.display);
    }
  ...
  getConstraints(de, path) {
    ...
    if (!constraints || constraints.length === 0) {
      ...
      constraints = this.getConstraintOnValue(value, this._specs.dataElements);
    }
